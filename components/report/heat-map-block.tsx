...
function buildForensicFlags(isSuspicious: boolean, details?: CaseDetails): FlagEntry[] {
  ...
  if (!isSuspicious && !isProSw && !isAiGen && !isSusEncoder) {
    return [{ type: "green", icon: "check", title: "No issues detected", description: "No errors found" }]
  }

  const out: FlagEntry[] = []
  if (isSuspicious) {
    out.push({ type: "red", icon: "info", title: "Deepfake detected", description: "AI-generated content detected by the model" })
  }
  if (isAiGen && !isSuspicious) {
    out.push({ type: "red", icon: "info", title: "AI generator detected", description: "Metadata indicates the video was generated by AI" })
  }
  if (isProSw) {
    out.push({ type: "amber", icon: "warn", title: "Professional software detected", description: "Metadata indicates professional editing software (e.g. Adobe After Effects)" })
  }
  if (isSusEncoder) {
    out.push({ type: "amber", icon: "warn", title: "Suspicious metadata", description: "Missing camera data, encoding mismatches, or signs of video conversion" })
  }
  return out
}
