...
// ── Forensic Flags (inline, data-driven) ───────────────────────────
interface FlagDef { type: "red" | "amber" | "green"; iconEl: React.ReactNode; title: string; description: string }
...
function infoIcon() {
  return (<svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, marginTop: "1px" }}><circle cx="7" cy="7" r="6" fill="#DC2626" /><text x="7" y="10.5" textAnchor="middle" fill="#fff" fontSize="9" fontWeight="700">i</text></svg>)
}
function warnIcon(color: string) {
  return (<svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, marginTop: "1px" }}><path d="M7 1L1 12h12L7 1z" fill={color} stroke={color} strokeWidth="0.5" /><text x="7" y="10.5" textAnchor="middle" fill="#fff" fontSize="8" fontWeight="700">!</text></svg>)
}
function checkIcon() {
...
function buildFlagsFromDetails(isSuspicious: boolean, details?: CaseDetails): FlagDef[] {
  ...
  if (!isSuspicious && !isAiGenerator && !isProfessionalSoftware && !hasSuspiciousEncoder) {
    return [{ type: "green", iconEl: checkIcon(), title: "No signs of manipulation detected", description: "No errors were found during analysis." }]
  }

  const flags: FlagDef[] = []
  if (isSuspicious) {
    flags.push({
      type: "red", iconEl: infoIcon(),
      title: "High confidence of AI-generated content detected",
      description: "Deepfake detected by the model. Signatures consistent with known deepfake generation tools found in file structure.",
    })
  }
  if (isAiGenerator) {
    flags.push({
      type: "red", iconEl: warnIcon("#DC2626"),
      title: "AI video generator signatures detected",
      description: provSoftware
        ? `Metadata contains clear signs that the video was generated by an AI tool. Software: ${provSoftware}.`
        : "Metadata contains clear signs that the video was generated by an AI tool.",
    })
  }
  if (isProfessionalSoftware) {
    flags.push({
      type: "amber", iconEl: warnIcon("#F59E0B"),
      title: "Professional video editing software detected",
      description: provSoftware
        ? `Metadata contains clear signs of professional software such as ${provSoftware}.`
        : "Metadata contains clear signs of professional software such as Adobe After Effects or similar tools.",
    })
  }
  if (hasSuspiciousEncoder) {
    flags.push({
      type: "amber", iconEl: warnIcon("#F59E0B"),
      title: "Suspicious metadata detected",
      description: encoder
        ? `Suspicious metadata found \u2014 encoder: ${encoder}. This can be a result of video conversions.`
        : "Suspicious metadata found \u2014 for example missing camera data, encoding mismatches, etc.",
    })
  }
  return flags
}

function ForensicFlagsInline({ isSuspicious, details }: { isSuspicious: boolean; details?: CaseDetails }) {
  const flags = buildFlagsFromDetails(isSuspicious, details)
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
      {flags.map((flag, i) => {
        const s = flagStyles[flag.type]
        return (
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "8px", padding: "8px 10px", background: s.bg, borderLeft: `4px solid ${s.border}`, borderRadius: "4px" }}>
            {flag.iconEl}
            <div>
              <div style={{ fontSize: "9px", fontWeight: 700, color: s.titleColor, lineHeight: "1.3" }}>{flag.title}</div>
              <div style={{ fontSize: "8px", color: s.descColor, lineHeight: "1.4", marginTop: "2px" }}>{flag.description}</div>
            </div>
          </div>
        )
      })}
...
// ── Extracted Metadata (inline, grouped) ───────────────────────────
interface DecodedMetadata { general?: Record<string, string>; video?: Record<string, string>; audio?: Record<string, string> }

function ExtractedMetadataInline({ meta }: { meta?: DecodedMetadata }) {
  const sections: { heading: string; rows: { label: string; value: string }[] }[] = []

  // Video Stream
  const videoRows: { label: string; value: string }[] = []
  if (meta?.video) {
    const v = meta.video
    if (v.codec_id || v.format) videoRows.push({ label: "Codec", value: v.format ? `${v.format} (${v.codec_id || ""})` : v.codec_id || "\u2014" })
    if (v.width && v.height) videoRows.push({ label: "Resolution", value: `${v.width} x ${v.height}` })
    if (v.frame_rate) videoRows.push({ label: "Frame Rate", value: v.frame_rate })
  }
  if (meta?.general) {
    if (meta.general.overall_bit_rate) videoRows.push({ label: "Bitrate", value: meta.general.overall_bit_rate })
    if (meta.general.duration) videoRows.push({ label: "Duration", value: meta.general.duration })
  }
  if (meta?.audio?.sampling_rate) videoRows.push({ label: "Sample Rate", value: meta.audio.sampling_rate })
  if (videoRows.length > 0) sections.push({ heading: "Video Stream", rows: videoRows })

  // Container
  const containerRows: { label: string; value: string }[] = []
  if (meta?.general?.format) containerRows.push({ label: "Format", value: meta.general.format })
  if (meta?.general?.writing_application) containerRows.push({ label: "Encoder", value: meta.general.writing_application })
  if (containerRows.length > 0) sections.push({ heading: "Container", rows: containerRows })
      ...
      {sections.map((section, si) => (
        <div key={section.heading}>
          ...
          {section.rows.map((r, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "3px 10px", background: i % 2 === 0 ? "#f8fafc" : "#ffffff", borderBottom: i < section.rows.length - 1 || si < sections.length - 1 ? "1px solid #f0f1f3" : "none" }}>
    ...
    <div style={{ marginBottom: "6px" }}>
      ...
      <div style={{ display: "flex", gap: "8px" }}>
        ...
        <div style={{ flex: "1.4", display: "flex", flexDirection: "column" }}>
          ...
          <ForensicFlagsInline isSuspicious={isSuspicious} details={details} />
        </div>

        {/* RIGHT: Extracted Metadata */}
        <ExtractedMetadataInline meta={meta} />
      </div>
    </div>
  )
}
